#[cfg(test)]
mod tests {
    use bitcoin_dojo::ecc::util::{sha256, secure_random_bytes};
    ...
    fn test_secure_random_bytes_distribution() {
        ...
        assert!(zeros > expected - tolerance && zeros < expected + tolerance,
                "Zero count {} is outside expected range", zeros);
        assert!(ones > expected - tolerance && ones < expected + tolerance,
                "One count {} is outside expected range", ones);
    }
    ...
    fn test_sha256_avalanche_effect() {
        ...
        assert!(different_bits > 64, "Avalanche effect too weak: only {} bits different", different_bits);
    }
}
