use bitcoin_dojo::utils::base58::{encode_base58, decode_base58, encode_base58_check, decode_base58_check};
...
mod tests {
    ...
    fn test_encode_base58_alphabet_coverage() {
        ...
        assert!(found_chars.len() > 10, "Should produce diverse character set");
    ...
    fn test_encode_base58_incremental_values() {
        ...
        for i in 0u8..10u8 {
            ...
            assert!(results.insert(result.clone()), "Duplicate result for input ending with {}: {}", i, result);
        }
    }
    ...
    fn test_decode_base58_all_valid_characters() {
        ...
        for c in alphabet.chars() {
            ...
            assert!(!result.unwrap().is_empty(), "Decoded character should not be empty: {}", c);
        }
    }
    ...
    fn test_encode_base58_check_with_leading_zeros() {
        ...
        for input in test_cases {
            ...
            if input[0] == 0x00 {
                assert!(result.starts_with('1'), "Should start with '1' for leading zero in input: {:?}", input);
            }
    ...
    fn test_encode_base58_check_vs_regular_base58() {
        ...
        for input in test_cases {
            ...
            assert!(check_result.len() > regular_result.len(), 
                   "Base58Check should be longer for input: {:?}", input);
    ...
    fn test_encode_base58_check_incremental_data() {
        ...
        for i in 0u8..20u8 {
            ...
            assert!(results.insert(result.clone()), 
                   "Duplicate result for input ending with {}: {}", i, result);
        }
    }
    ...
    fn test_encode_decode_roundtrip_leading_zeros() {
        ...
        for original in test_cases {
            let encoded = encode_base58(&original);
            let decoded = decode_base58(&encoded).unwrap();
            assert_eq!(original, decoded, "Leading zero round-trip failed for: {:?}", original);
        }
    }
    ...
    fn test_encode_decode_roundtrip_large_data() {
        ...
        for original in test_cases {
            let encoded = encode_base58(&original);
            let decoded = decode_base58(&encoded).unwrap();
            assert_eq!(original, decoded, "Large data round-trip failed for length: {}", original.len());
        }
    }
    ...
    fn test_encode_decode_roundtrip_random_patterns() {
        ...
        for pattern in test_patterns {
            ...
            let decoded = decode_base58(&encoded).unwrap();
            assert_eq!(pattern, decoded, "Pattern round-trip failed for: {:02x?}", pattern);
        }
    }
    ...
    fn test_encode_decode_roundtrip_edge_cases() {
        ...
        for edge_case in edge_cases {
            ...
            let decoded = decode_base58(&encoded).unwrap();
            assert_eq!(edge_case, decoded, "Edge case round-trip failed for: {:02x?}", edge_case);
        }
    }
    ...
    fn test_encode_decode_roundtrip_bitcoin_like_data() {
        ...
        let bitcoin_like_cases = vec![
            // P2PKH address payload (version + hash160 + checksum)
            {
                ...
                payload.extend_from_slice(&[0x89, 0xab, 0xcd, 0xef, 0xab, 0xba, 0xab, 0xba,
                                          0xab, 0xba, 0xab, 0xba, 0xab, 0xba, 0xab, 0xba,
            ...
            // P2SH address payload
            {
                ...
                payload.extend_from_slice(&[0x12, 0x34, 0x56, 0x78, 0x9a, 0xbc, 0xde, 0xf0,
                                          0x12, 0x34, 0x56, 0x78, 0x9a, 0xbc, 0xde, 0xf0,
    ...
    fn test_decode_base58_preserves_leading_zeros() {
        ...
        for (input, expected) in test_cases {
            ...
            assert_eq!(result, expected, "Leading zero preservation failed for input: {}", input);
        }
    }
    ...
    fn test_decode_base58_error_messages() {
        ...
        for (input, expected_error) in invalid_cases {
            ...
            assert_eq!(result.unwrap_err(), expected_error, "Wrong error message for input: {}", input);
        }
    }
    ...
    fn test_decode_base58_comprehensive_alphabet() {
        ...
        for (i, c) in alphabet.chars().enumerate() {
            ...
            assert_eq!(result.len(), 1, "Character '{}' should decode to single byte", c);
            assert_eq!(result[0] as usize, i, "Character '{}' should decode to value {}", c, i);
        }
    }
    ...
    fn test_decode_base58_performance_large_input() {
        ...
        assert!(!result.unwrap().is_empty(), "Large input should produce non-empty output");
    }
    ...
    fn test_base58_check_avalanche_effect() {
        ...
        for i in 0..base_data.len() {
            ...
            // Should be different
            assert_ne!(base_encoded, modified_encoded, 
                      "Output should change when byte {} changes", i);
            ...
            let different_chars = base_chars.iter()
            ...
            assert!(different_chars > 0, 
                   "Should have character differences when byte {} changes", i);
        }
    }
    ...
    fn test_decode_base58_check_too_short() {
        ...
        assert_eq!(result.unwrap_err(), "Base58Check data too short: must be at least 4 bytes");
    }
    ...
    fn test_decode_base58_check_invalid_checksum() {
        ...
        *last_char = if *last_char == '2' { '3' } else { '2' };
        ...
        assert!(result.is_err());
        assert_eq!(result.unwrap_err(), "Base58Check checksum verification failed");
    }
    ...
    fn test_decode_base58_check_bitcoin_addresses() {
        ...
        let address_cases = vec![
            // P2PKH mainnet
            {
                ...
                addr.extend_from_slice(&[0x89, 0xab, 0xcd, 0xef, 0x01, 0x23, 0x45, 0x67,
                                       0x89, 0xab, 0xcd, 0xef, 0x01, 0x23, 0x45, 0x67,
            ...
            // P2SH mainnet
            {
                ...
                addr.extend_from_slice(&[0x12, 0x34, 0x56, 0x78, 0x9a, 0xbc, 0xde, 0xf0,
                                       0x12, 0x34, 0x56, 0x78, 0x9a, 0xbc, 0xde, 0xf0,
                                       0x12, 0x34, 0x56, 0x78]); // 20-byte hash
                addr
            },
    ...
    fn test_decode_base58_check_corruption_detection() {
        ...
        let original_data = vec![0x00, 0x12, 0x34, 0x56, 0x78, 0x9a, 0xbc, 0xde, 0xf0,
        ...
        for pos in 0..valid_encoded.len() {
            ...
            let alphabet = "123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz";
            let replacement_char = alphabet.chars()
            ...
            assert!(result.is_err(), 
                   ...
                   pos, original_char, replacement_char);
        }
    }
    ...
    fn test_decode_base58_check_edge_cases() {
        ...
        for edge_case in edge_cases {
            ...
            let decoded = decode_base58_check(&encoded).unwrap();
            assert_eq!(edge_case, decoded, "Edge case decode failed for: {:02x?}", edge_case);
        }
    }
    ...
    fn test_decode_base58_check_large_data() {
        ...
        for large_data in large_cases {
            ...
            let decoded = decode_base58_check(&encoded).unwrap();
            assert_eq!(large_data, decoded, "Large data decode failed for length: {}", large_data.len());
        }
    }
    ...
    fn test_decode_base58_check_vs_regular_base58_security() {
        ...
        let original_data = vec![0x00, 0x89, 0xab, 0xcd, 0xef, 0x01, 0x23, 0x45, 0x67,
        ...
        if regular_corrupted.len() > 0 && check_corrupted.len() > 0 {
            ...
            assert!(regular_result.is_ok(), "Regular Base58 should decode corrupted data");
            
            // Base58Check should detect the corruption
            let check_result = decode_base58_check(&check_corrupted_string);
            assert!(check_result.is_err(), "Base58Check should detect corruption");
        }
    }
    ...
    fn test_decode_base58_check_checksum_boundary() {
        ...
        assert!(result2.is_err(), "Should detect corruption in checksum portion");
    }
    ...
    fn test_decode_base58_check_error_propagation() {
        ...
        for invalid_input in invalid_base58_inputs {
            let result = decode_base58_check(invalid_input);
            assert!(result.is_err(), "Should propagate Base58 decode error for: {}", invalid_input);
            // The error should be from Base58 decoding, not checksum verification
            let error_msg = result.unwrap_err();
            assert!(error_msg.contains("Invalid character"), "Should be Base58 decode error, got: {}", error_msg);
        }
    }
    ...
    fn test_decode_base58_check_leading_zeros_preservation() {
        ...
        for original in test_cases {
            let encoded = encode_base58_check(&original);
            let decoded = decode_base58_check(&encoded).unwrap();
            assert_eq!(original, decoded, "Leading zeros not preserved for: {:?}", original);
            
            // Verify the encoded version starts with '1' for leading zeros
            if original[0] == 0x00 {
                assert!(encoded.starts_with('1'), "Should start with '1' for leading zero");
            }
        }
    }
    ...
    fn test_decode_base58_check_all_zero_data() {
        ...
        assert!(ones_count >= 20, "Should have at least 20 leading '1's for 20 zero bytes");
    }
    ...
    fn test_decode_base58_check_single_bit_errors() {
        ...
        for byte_idx in 0..original_data.len() {
            for bit_idx in 0..8 {
                ...
                assert!(result.is_err(), 
                       "Should detect single bit flip at byte {} bit {}", byte_idx, bit_idx);
            }
        }
    }
    ...
    fn test_decode_base58_check_checksum_collision_resistance() {
        ...
        for i in 0..base_data.len() {
            for delta in 1u8..=10u8 {
                ...
                let modified_encoded = encode_base58_check(&modified_data);
                assert!(encoded_results.insert(modified_encoded.clone()), 
                       "Checksum collision detected for byte {} delta {}", i, delta);
